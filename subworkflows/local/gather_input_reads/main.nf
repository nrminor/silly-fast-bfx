include { samplesheetToList } from 'plugin/nf-schema'
include { DOWNLOAD_SRACHA_FASTQ } from '../../../modules/local/download_sracha_fastq'
include { PRESERVE_SAMPLESHEET } from '../../../modules/local/preserve_samplesheet'
include { VALIDATE_SRACHA_RUN   } from '../../../modules/local/validate_sracha_run'

def normalizePlatform(platform) {
    def normalized_platform = platform.toString().toLowerCase()

    normalized_platform in ['nanopore', 'oxford_nanopore', 'oxford-nanopore']
        ? 'ont'
        : normalized_platform
}

workflow GATHER_INPUT_READS {
    take:
    samplesheet

    main:
    rows = samplesheetToList(
        samplesheet,
        "${projectDir}/assets/samplesheet_schema.json",
    )

    PRESERVE_SAMPLESHEET(samplesheet)

    ch_input_rows = channel
        .fromList(rows)
        .map { meta, srr, fastq1, fastq2, fastq1_glob, fastq2_glob ->
            tuple(
                meta + [platform: normalizePlatform(meta.platform)],
                srr ?: '',
                fastq1 ?: '',
                fastq2 ?: '',
                fastq1_glob ?: '',
                fastq2_glob ?: '',
            )
        }
        .branch { _meta, srr, fastq1, _fastq2, fastq1_glob, _fastq2_glob ->
            exact: fastq1
            grouped: fastq1_glob
            sra: srr
        }

    ch_exact_reads = ch_input_rows.exact.map {
        meta, _srr, fastq1, fastq2, _fastq1_glob, _fastq2_glob ->

        tuple(
            meta + [
                read_set: 'input',
            ],
            [fastq1, fastq2].findAll(),
        )
    }

    ch_grouped_reads = ch_input_rows.grouped.map {
        meta, _srr, _fastq1, _fastq2, fastq1_glob, fastq2_glob ->

        def r1 = [files(fastq1_glob, checkIfExists: true)].flatten().sort {
            left, right -> left.toString() <=> right.toString()
        }
        def r2 = fastq2_glob
            ? [files(fastq2_glob, checkIfExists: true)].flatten().sort {
                left, right -> left.toString() <=> right.toString()
            }
            : []

        tuple(
            meta + [
                read_set: 'input',
            ],
            r1 + r2,
        )
    }

    ch_sra_runs = ch_input_rows.sra.map {
        meta, srr, _fastq1, _fastq2, _fastq1_glob, _fastq2_glob ->

        tuple(meta, srr.toUpperCase())
    }

    VALIDATE_SRACHA_RUN(ch_sra_runs)
    DOWNLOAD_SRACHA_FASTQ(VALIDATE_SRACHA_RUN.out.runs)

    ch_sra_reads = DOWNLOAD_SRACHA_FASTQ.out.reads.map { meta, reads ->
        def ordered_reads = [reads].flatten().sort {
            left, right -> left.toString() <=> right.toString()
        }

        tuple(
            meta + [
                read_set: 'input',
            ],
            ordered_reads,
        )
    }

    ch_input_reads = ch_exact_reads
        .mix(ch_grouped_reads)
        .mix(ch_sra_reads)

    emit:
    reads = ch_input_reads
    samplesheet = PRESERVE_SAMPLESHEET.out.samplesheet
}
